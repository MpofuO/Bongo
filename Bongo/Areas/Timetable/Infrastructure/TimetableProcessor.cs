...
namespace Bongo.Areas.TimetableArea.Infrastructure
{
    public class TimetableProcessor
    {
        ...
        public Session[,] GetSessionsArray(out List<List<Session>> Clashes, out List<Lecture> GroupedLectures)
        {
            Clashes = GetClashingSessions();
            GroupedLectures = GetGroupedLectures();
        ...
        public List<Lecture> GetGroupedLectures(bool ignoreChosen = false, bool ignoreIgnored = false)
        {
        ...
        public List<List<Session>> GetClashingSessions(bool ignoreChosen = false)
        {
            ...
            for (int i = 0; i < lstSessions.Count; i++)
            {
                ...
                for (int j = i + 1; j < lstSessions.Count; j++)
                {
                    ...
                    if (SessionControlHelpers.ContainsClashes(new string[] { lstSessions[i].sessionInPDFValue, lstSessions[j].sessionInPDFValue }))
                    {
        ...
        {
            List<List<Session>> allClashes = GetClashingSessions(true);
        ...
        private static void FilterBySemester(bool isForFirstSemester)
        {
            ...
            foreach (Lecture lect in groupedLectures)
            {
                ...
                for (int i = 0; i < lect.sessions.Count; i++)
                {
                    ...
                    for (int j = i + 1; j < lect.sessions.Count; j++)
                    {
                        ...
                        if (!SessionControlHelpers.ContainsClashes(new string[] { lect.sessions[i].sessionInPDFValue, lect.sessions[j].sessionInPDFValue }))
                        {
                            AllSameTime = false;
                            break;
                        }
                    ...
                    if (AllSameTime)
                    {
                        Session clashing = lstSessions.FirstOrDefault(s => SessionControlHelpers.ContainsClashes(new string[] { s.sessionInPDFValue, lect.sessions[0].sessionInPDFValue }));
                        if (clashing != null)
                        {
